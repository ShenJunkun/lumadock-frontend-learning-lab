...
export function createLeadSubmissionState(productId?: string): LeadSubmissionState {
  return {
    message: null,
...
export function leadSubmissionReducer(
  ...
  switch (action.type) {
    case "reset":
      return createLeadSubmissionState(action.productId ?? state.productId ?? undefined);
    case "submit":
      return {
        message: null,
