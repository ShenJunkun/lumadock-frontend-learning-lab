...
export const handlers = [
  ...
  http.post("*/api/auth/login", async ({ request }) => {
    ...
    if (body.email === "admin@lumadock.local" && body.password === "admin123") {
      return HttpResponse.json(
        buildTokenResponse(mockUsers.admin, mockTokens.admin),
      );
    }
    if (
      body.email === "viewer@lumadock.local" &&
      body.password === "viewer123"
    ) {
      return HttpResponse.json(
        buildTokenResponse(mockUsers.viewer, mockTokens.viewer),
      );
    }
    return HttpResponse.json(
