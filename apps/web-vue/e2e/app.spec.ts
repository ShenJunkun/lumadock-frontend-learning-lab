...
test("home, catalog, detail, and booking flow", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "LumaDock", exact: true }),
  ).toBeVisible();
  await page.getByRole("link", { name: /Explore products/i }).click();
  await expect(
    page.getByRole("heading", { name: "LumaDock lineup" }),
  ).toBeVisible();

  await page.getByRole("link", { name: /LumaDock Studio/i }).click();
  await expect(
    page.getByRole("heading", { name: "LumaDock Studio" }),
  ).toBeVisible();
...
test("configurator priority can be reordered from the keyboard", async ({
  page,
}) => {
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.goto("/");

  const priorityList = page
    .getByRole("list", { name: "Configuration priority" })
    .first();
  await expect(priorityList.getByRole("listitem").nth(1)).toContainText(
    "Stand",
  );

  await priorityList
    .getByRole("button", { name: "Move Profile earlier" })
    .focus();
...
test("home page supports keyboard entry and has no critical a11y violations", async ({
  page,
}) => {
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.goto("/");
