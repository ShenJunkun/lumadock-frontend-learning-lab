...
本项目在 `apps/web/package.json` 中使用 `antd@^6.3.7`，当前锁定安装版本是 `6.3.7`。Ant Design 在这个项目里主要负责两件事：
...
一句话记忆：**antd 负责复杂业务组件和全局反馈，Tailwind / CSS 负责页面排版和品牌外观，`@lumadock/ui` 负责项目自己的可复用展示组件。**
