...
4. React 主线：组件拆分、props、状态、路由页面，见 [03 React 组件与路由](03-react-components-routing.md)；Vue 对照：SFC、template、props、Vue Router 页面，见 [17 Vue 组件与路由](17-vue-components-routing.md)。
...
读 React 页面骨架时，可以配合 [16 AppShell 逐段阅读指南](16-app-shell-reading-guide.md) 学习 JSX 标签来源、React Router 导航、登录态条件渲染和 Ant Design 使用边界。读 Vue 并行实现时，先看 [17 Vue 组件与路由](17-vue-components-routing.md)，再重点对照 `apps/web-vue`：`App.vue` 里的 `<RouterView />`、组件里的 `<template>` / `<script setup>`、Pinia store、Vue Router 守卫、Vue Query hook，以及 `packages/ui-vue` 中和 `packages/ui` 对应的共享组件。
