...
在 React / TypeScript 项目中，“全局变量”并不是唯一答案。这个项目更推荐按数据来源和生命周期选择机制：局部状态、模块常量、Zustand、React Query、浏览器存储、环境变量、CSS 变量和 Provider 各自负责不同层次。Vue 并行应用沿用同一套判断，只是把 React 生态里的 Zustand、React Query、React Hook Form 分别换成 Pinia、Vue Query、Vue reactive state + Zod。
...
| React 主线                | Vue 并行实现                    | 项目落点                                                      |
| ------------------------- | ------------------------------- | ------------------------------------------------------------- |
| `useState` / `useReducer` | `ref` / `reactive` / `computed` | 搜索词、表单输入、提交状态                                    |
| Zustand                   | Pinia                           | `apps/web-vue/src/stores/*`                                   |
| React Query               | Vue Query                       | `@tanstack/vue-query` 的 `useQuery` / `useMutation`           |
| React Hook Form + Zod     | Vue `reactive` + Zod            | `LeadForm.vue` 和 `leadFormSchema.ts`                         |
| React Provider            | `app.use(...)` + Vue 插件       | `main.ts` 安装 Router、Pinia、Vue Query、Ant Design Vue、i18n |
| `children`                | `<slot />`                      | Vue 版 `AppShell` 接收页面内容                                |
...
Vue 并行应用的全局入口是 `apps/web-vue/src/main.ts`。它不会写一层层 React Provider，而是创建应用后安装插件：
...
这和 React Provider 的目标相同：让页面组件能读到路由、状态、请求缓存、组件库主题和语言环境。区别在于 React 用 JSX 组件包裹子树，Vue 用 `app.use(...)` 把插件注册到应用实例。
...
浏览器 Vue 前端：http://127.0.0.1:5174
  -> fetch HTTP 请求
...
| 层级            | 文件                                                                                                             | 作用                                                                                                     |
| --------------- | ---------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 页面和组件      | `apps/web/src/pages/*`、`apps/web/src/components/*`、`apps/web-vue/src/pages/*`、`apps/web-vue/src/components/*` | 调用 `useProducts()`、`useQuery(...)`、`useMutation(...)` 等 hook 或 API 函数，处理 loading、error、data |
| Web API 包装    | `apps/web/src/api/*`、`apps/web-vue/src/api/*`                                                                   | 给 React / Vue 页面提供更贴近业务的函数和 hooks，例如 `useProducts()`、`login()`、`submitLead()`         |
...
React 前端开发服务器在 `5173`，Vue 前端开发服务器在 `5174`，后端在 `8001`，端口不同就是跨源请求。后端在 `apps/api/app/main.py` 配置了 CORS：
...
所以浏览器默认允许 React 前端页面请求本地 FastAPI。Vue 并行应用如果直接请求真实后端，需要把 `CORS_ORIGINS` 扩展到 `http://127.0.0.1:5174,http://localhost:5174`，或在 mock 模式下先完成纯前端学习。

### Mock 和真实后端

React 的 `main.tsx` 和 Vue 的 `main.ts` 都会根据环境变量决定是否启用 MSW mock：
...
如果启用 mock，`apps/web/src/mocks/handlers.ts` 或 `apps/web-vue/src/mocks/handlers.ts` 会拦截 `/api/products`、`/api/leads`、`/api/auth/login` 等请求，返回前端本地模拟数据。这样即使 FastAPI 没启动，也能练习页面交互。

如果不启用 mock，请求就会走真实链路：

```text
React / Vue 页面 -> api-client fetch -> FastAPI -> SQLite
```
...
Vue 版同样不要把接口数据塞进 Pinia，除非它已经变成明确的客户端草稿。产品列表、产品详情、后台 leads 和统计数据交给 Vue Query；配置器、登录态、语言和主题交给 Pinia；预约表单当前输入交给 `reactive` 表单对象和 Zod 校验。
...
前端环境变量在 `apps/web/.env.*` 和 `apps/web-vue/.env.*`：
