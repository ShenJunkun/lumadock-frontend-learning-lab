...
| 模块                  | 要学什么                                                  | 验收方式                                       |
| --------------------- | --------------------------------------------------------- | ---------------------------------------------- |
| PWA / 离线能力        | manifest、离线页、生产环境 Service Worker 注册策略        | React / Vue 单测验证注册策略，构建包含静态资源 |
| Web Vitals / 性能预算 | 用统一预算描述 LCP、CLS、INP 和 bundle 风险               | 单测验证预算评级和事件记录                     |
| 安全与隐私加固        | CSP 学习、错误日志脱敏、敏感字段策略                      | 单测验证错误报告不会泄漏 PII                   |
| 可视化回归测试        | 用 Playwright screenshot 做关键页面视觉保护               | React / Vue 桌面和移动截图基线可稳定比对       |
| 设计系统深化          | 把 token 和基础组件沉到 `packages/ui` / `packages/ui-vue` | React / Vue 页面复用各自共享 UI 组件           |
| 状态建模              | 用 reducer、Pinia 或显式提交状态明确复杂流程              | 单测覆盖预约提交状态迁移                       |
...
React 截图基线覆盖 desktop 和 mobile 两个 Playwright project。更新基线时使用：

```powershell
npm.cmd run web:e2e -- e2e/visual.spec.ts --update-snapshots
```

Vue 并行应用也保留独立视觉快照，用来人工对比 React 基线布局。更新时使用：

```powershell
npm.cmd run vue:e2e -- e2e/visual.spec.ts --update-snapshots
```

两套快照允许可见文案里的 `React` -> `Vue` 差异，但页面层级、关键间距、表单状态、后台布局、移动端断点和 404 结构应该保持一致。
...
Vue 版对应 `packages/ui-vue`，导出 `designTokens`、`SurfaceCard`、`EmptyState`、`StatusBadge` 和 `LearningArchitecturePanel`。两个包不强行共用组件源码，而是共享命名、设计 token 和视觉语义：React 组件服务 `apps/web`，Vue 组件服务 `apps/web-vue`。
...
Vue 版用 `reactive` 表单对象、Zod 校验和 Vue Query mutation 表达同一条提交流程；配置器、认证、偏好设置放在 Pinia store 中。对照学习时重点看：React 的 reducer 更像事件驱动状态机，Vue 的 `reactive` / `computed` / `watch` 更直接体现响应式依赖，但两者都应该避免把 `isLoading`、`isSuccess`、`error` 等布尔组合散落在模板里。
